    ...
    pub fn next_token(&mut self) -> Token {
        ...
        match self.inner.next() {
            Some(Ok(kind)) => {
                ...
                Token::new(kind, Span::new(span.start, span.end, self.file_id))
            }
            Some(Err(())) => {
                let span = self.inner.span();
                Token::new(
                    TokenKind::Error,
                    Span::new(span.start, span.end, self.file_id),
                )
            }
            None => {
                self.at_eof = true;
        ...
        if c == '\\' {
            match chars.next() {
                ...
                Some('u') => {
                    ...
                    let c = char::from_u32(code).ok_or(StringParseError::InvalidUnicodeEscape)?;
                    result.push(c);
                }
...
mod tests {
    ...
    fn test_punctuation() {
        ...
        assert_eq!(
            lex(".. ..="),
            vec![TokenKind::DotDot, TokenKind::DotDotEq, TokenKind::Eof]
        );
    }
