...
pub fn register(vm: &mut VM) {
    // Ok constructor
    vm.define_native("Ok", 1, |args| Ok(ok(args[0].clone())));

    // Err constructor
    vm.define_native("Err", 1, |args| Ok(err(args[0].clone())));

    // is_ok
    vm.define_native("is_ok", 1, |args| Ok(Value::Bool(is_ok(&args[0]))));

    // is_err
    vm.define_native("is_err", 1, |args| Ok(Value::Bool(is_err(&args[0]))));

    // unwrap_result (different name to avoid conflict with Option.unwrap)
    vm.define_native("unwrap_ok", 1, |args| match &args[0] {
        Value::Enum(e) if e.enum_name == "Result" && e.variant == "Ok" => {
    ...
    // unwrap_err
    vm.define_native("unwrap_err", 1, |args| match &args[0] {
        Value::Enum(e) if e.enum_name == "Result" && e.variant == "Err" => {
            Ok(e.fields.first().cloned().unwrap_or(Value::Unit))
        }
        Value::Enum(e) if e.enum_name == "Result" && e.variant == "Ok" => {
    ...
    vm.define_native("result_unwrap_or", 2, |args| match &args[0] {
        Value::Enum(e) if e.enum_name == "Result" && e.variant == "Ok" => {
            Ok(e.fields.first().cloned().unwrap_or(Value::Unit))
        }
        _ => Ok(args[1].clone()),
    ...
    vm.define_native("result_ok", 1, |args| match &args[0] {
        Value::Enum(e) if e.enum_name == "Result" && e.variant == "Ok" => {
    ...
    vm.define_native("result_err", 1, |args| match &args[0] {
        Value::Enum(e) if e.enum_name == "Result" && e.variant == "Err" => {
            let val = e.fields.first().cloned().unwrap_or(Value::Unit);
            Ok(super::option::some(val))
        }
