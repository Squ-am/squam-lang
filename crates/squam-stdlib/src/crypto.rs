...
pub fn register(vm: &mut VM) {
    ...
    vm.define_native("base64_encode", 1, |args| {
        match &args[0] {
            Value::String(s) => {
    ...
    vm.define_native("base64_decode", 1, |args| {
        match &args[0] {
            Value::String(s) => {
    ...
    vm.define_native("base64_encode_url", 1, |args| {
        match &args[0] {
            Value::String(s) => {
    ...
    vm.define_native("base64_decode_url", 1, |args| {
        match &args[0] {
            Value::String(s) => {
                match general_purpose::URL_SAFE.decode(s.as_bytes()) {
                    Ok(bytes) => {
                        match String::from_utf8(bytes) {
                            Ok(decoded) => Ok(Value::String(Rc::new(decoded))),
    ...
    vm.define_native("hash_djb2", 1, |args| {
        match &args[0] {
            Value::String(s) => {
    ...
    vm.define_native("hash_fnv1a", 1, |args| {
        match &args[0] {
            Value::String(s) => {
    ...
    vm.define_native("hex_encode", 1, |args| {
        match &args[0] {
            Value::String(s) => {
    ...
    vm.define_native("hex_decode", 1, |args| {
        match &args[0] {
            Value::String(s) => {
                let s = s.as_str();
                if s.len() % 2 != 0 {
