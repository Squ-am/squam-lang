...
fn create_response_struct(status: i64, body: String, headers: HashMap<String, String>) -> Value {
    ...
    for (k, v) in headers {
        headers_instance
            .fields()
            .borrow_mut()
            .insert(k, Value::String(Rc::new(v)));
    }
...
pub fn register(vm: &mut VM) {
    ...
    vm.define_native("http_get", 1, |args| match &args[0] {
        Value::String(url) => match ureq::get(url.as_str()).call() {
            Ok(response) => {
    ...
    vm.define_native("http_get_text", 1, |args| match &args[0] {
        Value::String(url) => match ureq::get(url.as_str()).call() {
            Ok(response) => {
                let body = response.into_string().unwrap_or_default();
    ...
    vm.define_native("http_post", 2, |args| match (&args[0], &args[1]) {
    ...
    vm.define_native("http_post_json", 2, |args| match (&args[0], &args[1]) {
        (Value::String(url), Value::String(body)) => {
            match ureq::post(url.as_str())
                .set("Content-Type", "application/json")
                .send_string(body.as_str())
    ...
    vm.define_native("http_put", 2, |args| match (&args[0], &args[1]) {
        (Value::String(url), Value::String(body)) => {
    ...
    vm.define_native("http_delete", 1, |args| match &args[0] {
        Value::String(url) => match ureq::delete(url.as_str()).call() {
            Ok(response) => {
                let status = response.status() as i64;
                let body = response.into_string().unwrap_or_default();
