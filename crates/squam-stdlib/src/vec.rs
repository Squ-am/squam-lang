...
pub fn register(vm: &mut VM) {
    ...
    vm.define_native("arr_first", 1, |args| match &args[0] {
        Value::Array(arr) => {
            ...
            arr.first()
                .cloned()
                .ok_or_else(|| "arr_first: array is empty".to_string())
        }
        Value::Tuple(t) => t
            .first()
            .cloned()
            .ok_or_else(|| "arr_first: tuple is empty".to_string()),
        other => Err(format!(
            "arr_first: expected array, got {}",
            other.type_name()
        )),
    });
    ...
    vm.define_native("arr_last", 1, |args| match &args[0] {
        Value::Array(arr) => {
            ...
            arr.last()
                .cloned()
                .ok_or_else(|| "arr_last: array is empty".to_string())
        }
        Value::Tuple(t) => t
            .last()
            .cloned()
            .ok_or_else(|| "arr_last: tuple is empty".to_string()),
        other => Err(format!(
            "arr_last: expected array, got {}",
            other.type_name()
        )),
    });
    ...
    vm.define_native("arr_is_empty", 1, |args| match &args[0] {
        ...
        other => Err(format!(
            "arr_is_empty: expected array, got {}",
            other.type_name()
        )),
    });
    ...
    vm.define_native("arr_clone", 1, |args| match &args[0] {
        ...
        other => Err(format!(
            "arr_clone: expected array, got {}",
            other.type_name()
        )),
    });

    // arr_concat(a: array, b: array) -> array
    vm.define_native("arr_concat", 2, |args| match (&args[0], &args[1]) {
    ...
    vm.define_native("arr_repeat", 2, |args| match &args[1] {
        ...
        other => Err(format!(
            "arr_repeat: expected int count, got {}",
            other.type_name()
        )),
    });

    // range(start: int, end: int) -> array
    vm.define_native("range", 2, |args| match (&args[0], &args[1]) {
    ...
    vm.define_native("arr_sort", 1, |args| match &args[0] {
        Value::Array(arr) => {
            arr.borrow_mut().sort_by(|a, b| value_cmp(a, b, false));
            Ok(Value::Unit)
        }
        other => Err(format!(
            "arr_sort: expected array, got {}",
            other.type_name()
        )),
    });
    ...
    vm.define_native("arr_sort_desc", 1, |args| match &args[0] {
        Value::Array(arr) => {
            arr.borrow_mut().sort_by(|a, b| value_cmp(a, b, true));
            Ok(Value::Unit)
        }
        other => Err(format!(
            "arr_sort_desc: expected array, got {}",
            other.type_name()
        )),
    });

    // arr_join(arr: array, sep: string) -> string
    vm.define_native("arr_join", 2, |args| match (&args[0], &args[1]) {
    ...
    vm.define_native("arr_flatten", 1, |args| match &args[0] {
        ...
        other => Err(format!(
            "arr_flatten: expected array, got {}",
            other.type_name()
        )),
    });
        ...
        Value::Array(arr) => {
            ...
            for item in arr.iter() {
                match (&min, item) {
                    ...
                    (Some(Value::Float(m)), Value::Float(n)) if n < m => {
                        min = Some(Value::Float(*n))
                    }
                    _ => {}
                }
            }
