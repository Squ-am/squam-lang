...
pub fn register(vm: &mut VM) {
    ...
    vm.define_native("mkdir_all", 1, |args| {
        match &args[0] {
            Value::String(path) => {
    ...
    vm.define_native("rmdir_all", 1, |args| {
        match &args[0] {
            Value::String(path) => {
    ...
    vm.define_native("remove_file", 1, |args| {
        match &args[0] {
            Value::String(path) => {
    ...
    vm.define_native("rename", 2, |args| {
        match (&args[0], &args[1]) {
    ...
    vm.define_native("copy_file", 2, |args| {
        match (&args[0], &args[1]) {
            (Value::String(from), Value::String(to)) => {
                match fs::copy(from.as_str(), to.as_str()) {
                    Ok(_) => Ok(Value::Bool(true)),
                    Err(_) => Ok(Value::Bool(false)),
                }
            }
    ...
    vm.define_native("read_dir", 1, |args| {
        match &args[0] {
            Value::String(path) => {
    ...
    vm.define_native("is_file", 1, |args| {
        match &args[0] {
            Value::String(path) => {
                Ok(Value::Bool(Path::new(path.as_str()).is_file()))
            }
    ...
    vm.define_native("is_dir", 1, |args| {
        match &args[0] {
            Value::String(path) => {
                Ok(Value::Bool(Path::new(path.as_str()).is_dir()))
            }
    ...
    vm.define_native("file_size", 1, |args| {
        match &args[0] {
            Value::String(path) => {
